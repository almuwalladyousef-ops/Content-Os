...
export async function GET(req: NextRequest) {
  ...
  if (nonce) {
    try {
      await stashHandoff(nonce, 'google', account)
      handoffOk = true
    } catch (e) {
  ...
  return NextResponse.redirect(new URL(`/settings?yt_connected=1${nonce ? (handoffOk ? '&handoff_done=1' : '&handoff_failed=1') : ''}`, req.url))
}
