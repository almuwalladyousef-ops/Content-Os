...
// ── Delivery dedup ───────────────────────────────────────────────────────────
// Meta's webhook platform is documented to redeliver the same event more than
// once, and recommends deduping by the event's own id. A redelivered tap used
// to re-run the whole send — sendDMToUser had already completed for the first
// delivery, but the second delivery's own hasBeenDMed check ran before that
// first delivery's logDM write landed, so both saw "not yet sent" and both
// sent. Claiming the event id up front, before any Instagram API calls,
// shrinks that race from "however long the send takes" to one Drive round trip.
const CLAIM_TTL_MS = 60 * 60 * 1000 // well past any realistic Meta redelivery gap

export async function claimEvent(key) {
  if (!key) return true // nothing stable to key on — can't dedupe, let it through
  const db = await readDB()
  db.processedEvents = db.processedEvents || {}

  const cutoff = Date.now() - CLAIM_TTL_MS
  for (const [k, ts] of Object.entries(db.processedEvents)) {
    if (new Date(ts).getTime() < cutoff) delete db.processedEvents[k]
  }

  if (db.processedEvents[key]) return false // already claimed: this is a duplicate delivery
  db.processedEvents[key] = new Date().toISOString()
  await writeDB(db)
  return true
}
